...
const EventDetailModal: React.FC<Props> = ({ visible, onClose, event }) => {
    ...
    const handleNavigate = async () => {
        try {
            ...
            // 1. Prioritize locationId for navigation
            if (event.locationId) {
                const docRef = doc(db, "markers", event.locationId);
                const snap = await getDoc(docRef);
                ...
                if (snap.exists()) {
                    ...
                    // Fallback if locationId is invalid, but still navigate
                    navigation.navigate("Maps", { category: "Event", name: event.title });
                }

                // 2. If no locationId, use lat and lng as a fallback
                ...
                // 3. If no coordinates are available at all
            } else {
                navigation.navigate("Maps", { category: "Event", name: event.title });
            }
