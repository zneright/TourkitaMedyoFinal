...
const EventCalendar = () => {
    ...
    // --- MODIFICATION START ---
    // This logic is now aligned with LandmarkMarkers to show only today's events.
    const now = new Date();
    ...
    const todaysEvents = events.filter((event) => {
        ...
        // 1. Skip events that have already ended (from LandmarkMarkers)
        ...
        // 2. Handle weekly recurring events (from LandmarkMarkers)
        if (recurrence.frequency === "weekly") {
            const daysOfWeek = (recurrence.daysOfWeek || []).map((d) =>
                d.toLowerCase().slice(0, 3)
            );
            return daysOfWeek.includes(todayDayName) && isWithinDateRange;
        }
        
        // 3. Handle all other types (daily, once, multi-day) by checking if today is within the event's date range.
