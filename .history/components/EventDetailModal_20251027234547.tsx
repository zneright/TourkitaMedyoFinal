...
const EventDetailModal: React.FC<Props> = ({ visible, onClose, event }) => {
    ...
    const handleNavigate = async () => {
        ...
        try {
            ...
            // Path 1: Event has its own specific coordinates
            if (typeof event.lat === 'number' && typeof event.lng === 'number') {
                const target: NavigationTarget = {
                    name: event.title,
                    category: "Events",
                    latitude: event.lat,   // ✅ FIX: Use 'latitude' property
                    longitude: event.lng,
                };
                navigation.navigate("Maps", target);

                // Path 2: Event is linked to a pre-defined landmark
            } else if (event.locationId) { // ✅ FIX: Correctly placed as 'else if'
                const docRef = doc(db, "markers", event.locationId);
                const snap = await getDoc(docRef);
                ...
                if (snap.exists()) {
                    ...
                    // Fallback if the locationId is invalid
                    console.warn("Invalid locationId:", event.locationId);
                    navigation.navigate("Maps", { category: "Events" });
                }

                // Path 3: Fallback for events with no location data
            } else {
                console.warn("Event has no location data to navigate to.");
                navigation.navigate("Maps", { category: "Events" });
            }

            onClose(); // Close the modal after initiating navigation
        } catch (err) {
