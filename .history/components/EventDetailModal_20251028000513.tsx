...
// --- EventType definition remains the same ---
type EventType = {
    id?: string;
    title: string;
    description?: string;
...
const EventDetailModal: React.FC<Props> = ({ visible, onClose, event }) => {
    // ... other hooks and state variables remain the same
    ...
    // --- Recurrence and Public Status logic remains the same ---
    ...
    // --- (2) IMPLEMENT THE NAVIGATION HANDLER ---
    const handleNavigate = async () => {
        ...
        try {
            // Priority 1: Check for direct lat/lng on the event object.
            if (typeof event.lat === 'number' && typeof event.lng === 'number') {
                latitude = event.lat;
                longitude = event.lng;
            }
            // Priority 2: Check for a custom address object with coordinates.
            else if (typeof event.customAddress === 'object' && event.customAddress?.latitude && event.customAddress?.longitude) {
                latitude = event.customAddress.latitude;
                longitude = event.customAddress.longitude;
            }
            // Priority 3: Fetch coordinates from the associated landmark ID.
            else if (event.locationId) {
                const docRef = doc(db, "markers", event.locationId);
            ...
            if (latitude !== null && longitude !== null) {
                // Use a universal Google Maps URL for cross-platform compatibility.
                const mapUrl = `https://www.google.com/maps/dir/?api=1&destination=${latitude},${longitude}`;

                // Check if the device can handle the URL and open it.
                const supported = await Linking.canOpenURL(mapUrl);
                if (supported) {
                    await Linking.openURL(mapUrl);
                } else {
                    alert("Unable to open map application.");
                }
            } else {
                alert("Location coordinates for this event are not available.");
            }
        } catch (error) {
            console.error("Failed to open maps:", error);
            alert("An error occurred while trying to open navigation.");
        } finally {
            setLoading(false);
        }
    };
