...
const EventDetailModal: React.FC<Props> = ({ visible, onClose, event }) => {
    ...
    const handleNavigate = async () => {
        try {
            ...
            // Prioritize locationId for navigation
            if (event.locationId) {
                const docRef = doc(db, "markers", event.locationId);
                const snap = await getDoc(docRef);
                if (snap.exists()) {
                    ...
                    navigation.navigate("Maps", { category: "Events", name: event.title });
                }
                // If no locationId, use lat and lng as a fallback
                ...
                // If no coordinates are available at all
            } else {
                navigation.navigate("Maps", { category: "Event", name: event.title });
            }
