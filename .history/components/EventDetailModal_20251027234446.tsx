...
const EventDetailModal: React.FC<Props> = ({ visible, onClose, event }) => {
    ...
    const handleNavigate = async () => {
        try {
            ...
            // This is the base object for navigation parameters
            const target = {
                name: event.title,
                category: "Events",
            };

            // Path 1: Event has specific lat/lng
            if (event.lat && event.lng) {
                target.lat = event.lat;
                target.longitude = event.lng;

                // --- FIX: This line was causing the error and has been removed ---
                // loadDirection(target); 

                // Now, this navigation call will execute correctly
                navigation.navigate("Maps", target);

                // Path 2: Event is linked to a pre-defined location
            } else if (event.locationId) {
                const docRef = doc(db, "markers", event.locationId);
                const snap = await getDoc(docRef);
                if (snap.exists()) {
                    const marker = snap.data();
                    setSelectedLandmark(marker); // Set landmark in context
                    ...
                    // Fallback if locationId is invalid
