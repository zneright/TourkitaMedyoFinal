...
// Only these are needed for the corrected logic
...
const EventCalendar = () => {
    ...
    // --- MODIFICATION START ---
    // This is the definitive, simplified logic that handles all cases correctly.
    const todaysEvents = events.filter((event) => {
        ...
        // 1. The primary and most important check: Is today within the event's active date range?
        const isInDateRange = isWithinInterval(now, { start: startDate, end: endDate });

        // If it's not in the date range, we can immediately say no.
        if (!isInDateRange) {
            return false;
        }

        // 2. If it IS in the date range, we check for an *additional* condition for weekly events.
        if (event.recurrence?.frequency === "weekly") {
            ...
            // It must also fall on the correct day of the week.
            return daysOfWeek.includes(todayDayName);
        }

        // 3. If it's not a weekly event but it passed the date range check,
        // it is happening today. This correctly handles "once", "daily", and multi-day events.
        return true;
    });
    // --- MODIFICATION END ---
