...
export default function LandmarkProvider({ children }: PropsWithChildren) {
    ...
    const setNavigationOnly = (destination: any) => {
        if (destination) {
            // Clear any previously selected landmark to hide the sheet.
            setSelectedLandmarkState(null);
            // Set the destination to draw the route and start tracking.
            setNavigationDestination(destination);
        }
    };

    // ✅ 3. MODIFY THE ORIGINAL `setSelectedLandmark`
    // This is called when a user taps a marker on the map.
    const setSelectedLandmark = (landmark: any) => {
        // Set the state to show the landmark sheet.
        setSelectedLandmarkState(landmark);
        // ALSO set the navigation destination to show the route.
        setNavigationDestination(landmark);
    };

    // ✅ 4. UPDATE useEffect HOOKS TO USE `navigationDestination`
    // This ensures arrival detection and route refreshing works for BOTH cases.
    useEffect(() => {
        let interval: any;
        if (navigationDestination) { // Changed from selectedLandmark
            interval = setInterval(async () => {
                try {
                    ...
                    const destCoords = {
                        latitude: navigationDestination.latitude, // Changed from selectedLandmark
                        longitude: navigationDestination.longitude, // Changed from selectedLandmark
                    };
                    const distanceMeters = haversine(userCoords, destCoords);
                    if (distanceMeters <= 30) {
                        setArrived(true);
                        setNavigationDestination(null); // Clear navigation on arrival
    ...
    }, [navigationDestination]); // Changed from selectedLandmark

    useEffect(() => {
        let refreshInterval: any;
        // This effect should trigger whenever the destination is set.
        if (navigationDestination && !arrived) {
            loadDirection(navigationDestination); // Load initial direction
            refreshInterval = setInterval(() => {
                loadDirection(navigationDestination);
            }, 15000);
        } else {
            // If there's no destination, clear the route.
            setDirection(null);
            setShowDirection(false);
        }
        return () => clearInterval(refreshInterval);
    }, [navigationDestination, mode, arrived]); // Changed from selectedLandmark
    ...
    return (
        <View style={{ flex: 1 }}>
            <LandmarkContext.Provider
                value={{
                    selectedLandmark,
                    setSelectedLandmark,  // For map taps (shows sheet + route)
                    setNavigationOnly,    // For event modal (shows route ONLY)
                ...
                <View style={styles.overlay}>
                    <View style={styles.messageBox}>
                        ...
                        <LottieView
                            ...
                            autoPlay
                            loop={false} // Set loop to false for success animation
